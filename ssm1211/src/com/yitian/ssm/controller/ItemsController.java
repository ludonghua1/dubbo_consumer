...
public class ItemsController {
    ...
    public String likeItems(HttpServletRequest request, Items items, HttpSession session,Integer pageNo,Integer pageSize){
        session.setAttribute("items",items);
        pageNo = pageNo==null?1:pageNo;
        pageSize = pageSize==null?10:pageSize;
        //开启分页拦截
        PageHelper.startPage(pageNo,pageSize);
        List<Items> itemsList =itemsService.findItemsLike(items);
        //使用封装类，对查询结果进行分页处理
        PageInfo<Items> page = new PageInfo<Items>(itemsList);
        //将分页对象page存入request域返回给jsp页面
        request.setAttribute("page",page);
