				...
				<div class="pagebody3_news" id="xiaoxi" style="height:500px;">
					...
					<ul>
						<c:forEach var="news" items="${page.list}" varStatus="status">
							<li style=""><a href="javascript:;">${news.title}</a><span><fmt:formatDate
					...
					<ul class="page">
					    ...
					    <a href="${ctx}/index/news.shtml?pageNo=${pageSize}&pageSize=10" style="background: #C30D23 none repeat scroll 0% 0%;color:#ffffff;">${pageSize}</a>
					     </c:when>
					   <c:otherwise>
					   <a href="${ctx}/index/news.shtml?pageNo=${pageSize}&pageSize=10" >${pageSize}</a>
					   </c:otherwise>
					   </c:choose>
