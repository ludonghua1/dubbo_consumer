...
<body>
<center>
    <a href="${path}/shoppingCart.shtml">我的购物车</a>
    <form action="${path}/items/likeItems.shtml">
        商品名称: <input type="text" name="itemsName" value="${itemsVo.itemsName}">
        价格区间: <input type="text" name="minPrice" value="${itemsVo.minPrice}">-
        <input type="text" name="maxPrice" value="${itemsVo.maxPrice}">
        <button onclick="like()">提交</button>
    </form>
    <table border="1">
        <tr>
            <th></th>
    ...
    <c:forEach begin="1" end="${page.pages}" var="pageNo">
        <c:if test="${flag==1}">
            <a href="${path}/items/likeItems.?pageNo=${pageNo}&itemsName=${itemsVo.itemsName}&minPrice=${itemsVo.minPrice}&maxPrice=${itemsVo.maxPrice}">${pageNo}</a>
        </c:if>
        <c:if test="${flag==0}">
